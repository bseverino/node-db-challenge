...
- [x] Explain the difference between `Relational Databases` and `SQL`.
  ...
  - Relational databases are like spreadsheets - they store data in tables with rows and column. SQL, or Structured Query Language, is a language used to interact with those databases and perform CRUD operations.

- [x] Why do tables need a `primary key`?

  - A primary key is used so that every entry has a single unique value, ensuring there will be no duplicates.
...
- [x] What do we need in order to have a _many to many_ relationship between two tables.
  ...
  - We need a third table which houses a foreign key for each table we want to connect.
